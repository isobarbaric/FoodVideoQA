...
def create_intervals_naive(
    food_data_video: list[list[str]], video_name: str, video_fps: int
) -> list[list[int]]:
    intervals = []
    curr_idx = 0
    while curr_idx < len(food_data_video):
        ...
        for food_str in food_data_video[curr_idx]:
            ...
            if food_next_idx != curr_idx + 1:
                curr_best.append(
                    [food_next_idx - curr_idx, [curr_idx, food_next_idx, food_str]]
                )
...
def create_intervals_optimized(
    food_data_video: list[list[str]], video_name: str, video_fps: int
) -> list[list[int]]:
    intervals = []
    curr_idx = 0
        ...
        for food_str in food_data_video[curr_idx]:
            ...
            for i in range(current_ending + TOLERANCE, len(food_data_video)):
                ...
                if food_next_idx != curr_idx + 1:
                    curr_best.append(
                        [
                            current_ending - curr_idx,
                            curr_next_idx - current_ending,
                            [curr_idx, current_ending, food_str],
                        ]
                    )
                    done = True
            ...
            if not done:
                curr_best.append(
                    [current_ending - curr_idx, 0, [curr_idx, current_ending, food_str]]
                )
...
def merge_intervals(intervals: list[list[int]]) -> list[list[int]]:
    ...
    while curr_idx < len(intervals) - 1:
        if (
            len(merged_intervals) == 0
            or intervals[curr_idx][2] != intervals[curr_idx + 1][2]
        ):
            ...
            merged_intervals.append(
                [intervals[curr_idx][0], intervals[curr_idx + 1][1], food_str]
            )
            curr_idx += 2
